...
::=
 match context
   case HESSIAN_CONTEXT(hessHT=SOME(hessHT)) then
     ...
     case SIMVAR(varKind=BackendDAE.SEED_VAR(b=b)) then
       let seedName = if b then 'seedVars1' else 'seedVars'
       <<hessian-><%seedName%>[<%index%>] /* <%escapeCComments(crefStrNoUnderscore(name))%> <%variabilityString(varKind)%> */>>
